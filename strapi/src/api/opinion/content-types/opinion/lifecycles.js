...
async function generateSlug(title, existingId = null) {
  ...
  if (!baseSlug) {
    baseSlug = "opinion" // Or some other default like 'untitled'
  }
...
module.exports = {
  ...
  async afterCreate(event) {
    ...
    if (result.rich_body && result.rich_body.length > 0) {
      try {
        const fullEntry = await global.strapi.entityService.findOne("api::opinion.opinion", result.id, {
          populate: ["rich_body"]
        })
        ...
        await global.strapi.entityService.update("api::opinion.opinion", result.id, {
          data: { 
            searchableContent,
